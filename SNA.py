...
def MakeGraph():
	...
	while (i <= hDiv):
		...
		Dline = [x,yT,x,yB]
		graph.create_line(Dline, fill = graphColor)
		txt = str(np.round(freq/1000000,5))+'M'
...
def AddTextInfo():
	...
	graph.create_text(graphLeftBuffer + 1*(graphWidth/10), yInfo, text = txt, fill=textLabelColor)
	
	if measMode == 0:
		txt = "Mode: Absolute"
	if measMode == 1:
		txt = "Mode: Relative"
	graph.create_text(graphLeftBuffer + 3*(graphWidth/10), yInfo, text = txt, fill=textLabelColor)
	...
	graph.create_text(graphLeftBuffer + 5*(graphWidth/10), yInfo, text = txt, fill=textLabelColor)
...
def BAbout():
	messagebox.showinfo("About", "This is where the about message \n\nwill go. Does this word wrap or does it just keep going on for a really really long time?")
	...
	def run(self):
		...
		while True:			
			...
			if len(self.adcValue) != 0:
				readings[self.n*2+1] = float(self.adcValue) * .0218 - 83.16 #0.0488 or 0.1953 - 83.998
				self.n = self.n + 1
...
b = ttk.Button(controlArea, text="About", width=10, command=BAbout)
